...
allgenes = set() # all genes having a GO term
...
for line in gotext:
    ...
    ''' Remark 1:
     We decided to directly examine those genes that is certain to have a GO term associated in order to reduce the computational time.
    ...
    allgenes.add(item["gene_stab_id"]) # Store all genes found in the GOannotation file
...
the genes associated to that category as well as the genes that are absent or present in the selection are counted.
